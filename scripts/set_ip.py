"""Script to set the IP address of a device."""
...
if found:
    ...
    with dai.DeviceBootloader(info) as bl:
        if key in ["1", "2"]:
            ...
            val = input(
                f"Flashing {mode} IPv4 {ipv4}, mask {mask}, gateway {gateway} to the POE device. Enter 'y' to confirm. "
            ).strip()
            if val != "y":
    ...
    print("No device found. Please check the device is connected and powered on.")
